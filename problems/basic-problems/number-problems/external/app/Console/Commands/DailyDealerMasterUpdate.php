...
class DailyDealerMasterUpdate extends Command {
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'daily:dealer_master_update';
    protected $tableName = 'tbl_dealer_masters';
     ...
     * @var string
     */
    protected $description = 'Update/Insert data in tbl_dealer_masters table in power bi database';
    ...
    public function handle() {
        ...
        $localDatabaseData             = $BI_DB_CONNECTION->table($this->tableName)->orderBy('dealer_id', 'DESC')->first();
        $queryDate                     = Carbon::now()->subDays(1)->format('Y-m-d');
        $productionDatabaseData        = $CRM_DB_CONNECTION->table($this->tableName)->where('dealer_id', '>', $localDatabaseData->dealer_id)->get();
        ...
        if ($productionDatabaseUpdatedData->count() > 0) {
            ...
            foreach ($productionDatabaseUpdatedData as $data) {
                $BI_DB_CONNECTION->table($this->tableName)->upsert($data, ['dealer_id']);
            }
        }
