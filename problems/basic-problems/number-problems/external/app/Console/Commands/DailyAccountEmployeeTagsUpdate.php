...
class DailyAccountEmployeeTagsUpdate extends Command {
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'daily:account_employee_tags_update';
    protected $tableName = 'tbl_account_employee_tags';
     ...
     * @var string
     */
    protected $description = 'Update/Insert data in tbl_account_employee_tags table in power bi database';
    ...
    public function handle() {
        ...
        $localDatabaseData             = $BI_DB_CONNECTION->table($this->tableName)->orderBy('map_id', 'DESC')->first();
        $queryDate                     = Carbon::now()->subDays(1)->format('Y-m-d');
        $productionDatabaseData        = $CRM_DB_CONNECTION->table($this->tableName)->where('map_id', '>', $localDatabaseData->map_id)->get();
        ...
        if ($productionDatabaseUpdatedData->count() > 0) {
            ...
            foreach ($productionDatabaseUpdatedData as $data) {
                $BI_DB_CONNECTION->table($this->tableName)->upsert($data, ['map_id']);
            }
        }
