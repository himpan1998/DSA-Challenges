...
class DailyCampaignPurposesUpdate extends Command {
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'daily:campaign_purposes_update';
    protected $tableName = 'tbl_campaign_purposes';
     ...
     * @var string
     */
    protected $description = 'Update/Insert data in tbl_campaign_purposes table in power bi database';
    ...
    public function handle() {
        ...
        $localDatabaseData             = $BI_DB_CONNECTION->table($this->tableName)->orderBy('purpose_id', 'DESC')->first();
        $queryDate                     = Carbon::now()->subDays(1)->format('Y-m-d');
        $productionDatabaseData        = $CRM_DB_CONNECTION->table($this->tableName)->where('purpose_id', '>', $localDatabaseData->purpose_id)->get();
        ...
        if ($productionDatabaseUpdatedData->count() > 0) {
            ...
            foreach ($productionDatabaseUpdatedData as $data) {
                $BI_DB_CONNECTION->table($this->tableName)->upsert($data, ['purpose_id']);
            }
        }
