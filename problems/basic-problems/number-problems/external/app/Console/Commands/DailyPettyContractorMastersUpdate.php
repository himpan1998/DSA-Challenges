...
class DailyPettyContractorMastersUpdate extends Command {
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'daily:petty_contractor_masters_update';
    protected $tableName = 'tbl_petty_contractor_masters';
     ...
     * @var string
     */
    protected $description = 'Update/Insert data in tbl_petty_contractor_masters table in power bi database';
    ...
    public function handle() {
        ...
        $localDatabaseData             = $BI_DB_CONNECTION->table($this->tableName)->orderBy('pcontractor_id', 'DESC')->first();
        $queryDate                     = Carbon::now()->subDays(1)->format('Y-m-d');
        $productionDatabaseData        = $CRM_DB_CONNECTION->table($this->tableName)->where('pcontractor_id', '>', $localDatabaseData->pcontractor_id)->get();
        ...
        if ($productionDatabaseUpdatedData->count() > 0) {
            ...
            foreach ($productionDatabaseUpdatedData as $data) {
                $BI_DB_CONNECTION->table($this->tableName)->upsert($data, ['pcontractor_id']);
            }
        }
